...
class VendorRecords(VendorRecordsTemplate):
  def __init__(self, name_filter: str = "", **properties):
    ...
    self.button_add_vendor.role = "new-button"
  ...
  def button_add_vendor_click(self, **event_args):
    open_form("VendorRecord", is_new=True)
  ...
  def _load_data(self):
    ...
    rows = anvil.server.call("list_vendors_by_name", name_sub)
    self.label_count.text = f"{len(rows)} vendor(s) returned"
    self.repeating_panel_vendors.items = rows
