...
class VendorRecord(VendorRecordTemplate):
  def __init__(self, vendor_id=None, is_new=False, **properties):
    ...
    if self.is_new:
      ...
      try:
        self.text_vendor_id.text = anvil.server.call("generate_next_vendor_id")
      except Exception as e:
        Notification(
          f"Could not auto-generate vendor ID: {e}", style="warning"
        ).show()

      # Seed contacts once on first entry
      self._maybe_prefill_contacts_once(force=True)
    else:
      doc = anvil.server.call("get_vendor", vendor_id)
      if not doc:
  ...
  def _bind_doc(self, doc: dict):
    ...
    def _addr_of(kind):
      return next(
        (
          ...
          if a.get("type") == kind and a.get("is_default")
        ),
        None,
      )
    ...
    for kind, prefix in (("billing", "b"), ("shipping", "s")):
      ...
      getattr(self, f"text_{prefix}_admin_area").text = (
        addr.get("administrative_area", "") or ""
      )
      getattr(self, f"text_{prefix}_postal_code").text = (
        addr.get("postal_code", "") or ""
      )

    self.drop_down_billing_country_code.selected_value = (
      _addr_of("billing") or {}
    ).get("country_code", "NZ")
    self.drop_down_shipping_country_code.selected_value = (
  ...
  def button_delete_click(self, **event_args):
    try:
      ...
      if not vid:
        Notification("No vendor_id to delete.", style="warning").show()
        return
      ok = anvil.server.call("delete_vendor", vid)
      if ok:
        Notification("Vendor deleted.", style="success").show()
        open_form("VendorRecords")
      else:
  ...
  def button_save_click(self, **event_args):
    try:
      ...
      doc = anvil.server.call("upsert_vendor", payload)
      Notification("✅ Vendor saved.", style="success").show()
      open_form(VendorRecord(vendor_id=doc["vendor_id"], is_new=False))
    except Exception as e:
  ...
  def _wire_billing_sync_events(self):
    ...
    self.drop_down_billing_country_code.set_event_handler(
      "change", self._maybe_sync_shipping
    )
    self.check_box_same_as_billing.set_event_handler(
      "change", self._check_same_as_billing_changed
    )
  ...
  def _wire_contact_prefill_events(self):
    ...
    for src_attr, dest_attr, flag in pairs:
      ...
      if src:
        src.set_event_handler(
          "pressed_enter",
          lambda f=flag, s=src_attr, d=dest_attr, **ea: self._maybe_prefill_single(
            s, d, f
          ),
        )
        src.set_event_handler(
  ...
  def _maybe_prefill_contacts_once(self, force=False, **event_args):
    ...
    if force or all_blank:
      self._maybe_prefill_single(
        "text_name", "text_contact_name", "_contact_name_touched", force=True
      )
      self._maybe_prefill_single(
        "text_email", "text_contact_email", "_contact_email_touched", force=True
      )
      self._maybe_prefill_single(
        "text_phone_raw",
        "text_contact_phone_raw",
