...
class InventoryBinRow(InventoryBinRowTemplate):
  def __init__(self, **properties):
    ...
    self.text_box_part_id.set_event_handler("change", self.save_changes)
    self.text_box_part_name.set_event_handler("change", self.save_changes)
    ...
    self.drop_down_owner.items = ["Manufacturing", "Warehouse", "WIP", "Finished"]
    self.refresh_ui()

  def refresh_ui(self):
    self.text_box_bin_id.text = self.item.get("_id", "")
    self.text_box_part_id.text = self.item.get("part_id", "")
    self.text_box_part_name.text = self.item.get("part_name", "")
  ...
  def save_changes(self, **event_args):
    ...
    self.item["part_name"] = self.text_box_part_name.text.strip()
